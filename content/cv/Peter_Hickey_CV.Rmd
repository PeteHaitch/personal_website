...
n_distinct_ips <- sapply(pkgs, function(pkg) {
  sapply(years, function(year) {
    url <- paste0("http://bioconductor.org/packages/stats/bioc/", pkg, "/", 
                  pkg, "_", year, "_stats.tab")
    n <- try(read.table(url, header = TRUE)[13, "Nb_of_distinct_IPs"],
             silent = TRUE)
    if (is(n, "try-error")) {
      n <- NA
    }
...
- [bsseq](http://bioconductor.org/packages/bsseq/): Analyse, Manage and Store Bisulfite Sequencing Data
    - Number of downloads (2014-2017): `r n_downloads[, "bsseq"]`
- [DelayedMatrixStats](http://bioconductor.org/packages/DelayedMatrixStats/): Functions that Apply to Rows and Columns of 'DelayedMatrix' Objects
    - Number of downloads (2017): `r n_downloads["2017", "DelayedMatrixStats"]`
